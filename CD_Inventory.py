...
# KBrock, 2021-Mar-14, add code to handle tracks on an individual CD
#------------------------------------------#

import ProcessingClasses as PC
import IOClasses as IO

print ('\nWelcome to the Retro CD Inventory!  Let\'s do stuff ...\n')
...
while True:
    ...
    if strChoice == 'l':
        ...
        while True:
            if cd == None:
                print ('That CD does not exist.  Select [a] to add it! \n')
                break
            else:
                IO.ScreenIO.print_CD_menu()
                strChoice = IO.ScreenIO.menu_CD_choice()
                if strChoice == 'x':
                    print('Thanks for adding some tracks! \n')
                    break
                if strChoice == 'a':
                    ...
                    IO.ScreenIO.show_tracks(cd)
                    track_idx = input('Select the track you want to remove: ')
